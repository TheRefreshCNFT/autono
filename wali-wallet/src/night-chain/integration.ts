...
export class NightChainSecureStorage {
  ...
  /**
   * Get encrypted asset by ID
   * 
   * @param assetId Asset ID to retrieve
   * @returns Encrypted asset or null if not found
   */
  async getEncryptedAsset(assetId: string): Promise<EncryptedAsset | null> {
    return this.assetStorage.getEncryptedAsset(assetId);
  }

  /**
   * Decrypt an encrypted asset
   * 
   * @param asset Encrypted asset
   * @param accessKey User's access key
   * @returns Decrypted seed phrase bundle
   */
  async decryptAsset(asset: EncryptedAsset, accessKey: string): Promise<SeedPhraseBundle | null> {
    try {
      const decrypted = decryptSeedPhrases({
        encryptedData: asset.encryptedPayload,
        nonce: asset.nonce,
      ...
      console.error('[ERROR] Failed to decrypt asset:', error);
      return null;
    }
  }
