...
try {
    ...
    foreach ($asset in $data.asset_metrics) {
        ...
        # Check for pump (compare to 30 minutes ago)
        $thirtyMinAgo = $currentTime.AddMinutes(-$TimeWindow)
        $oldPrice = $recentHistory | Where-Object { 
            [DateTime]$_.timestamp -le $thirtyMinAgo 
    ...
    foreach ($alert in $alerts) {
        ...
        $message += "**Price:** $oldPriceStr → $newPriceStr ($changeStr in 30min)`n"
