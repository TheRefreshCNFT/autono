...
import { BitcoinAPI } from '@wallet-engine/bitcoin/api';
import { NightChainIntegration } from '@wallet-engine/night-chain/simple-adapter';
import { wipeMemory } from '@wallet-engine/utils/security';
  ...
  async createWallet(params: WalletCreationParams): Promise<WalletCreationResult> {
    ...
    try {
      ...
      if (params.chains.includes('bitcoin')) {
        ...
        const { BitcoinWallet } = await import('@wallet-engine/bitcoin/wallet');
